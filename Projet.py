...
I=6  #current
NB=300 # the nmber of coil
mass=1.44E-25# Rubidium87 mass
K = 1.38E-23  #Boltzmann constant
k=2*pi/780E-9 #wave number
T = 300 # around room temperature
ub=9.274E-24# Bohr Magneton
u0=1E-6 #Permeability
h=1E-34 #Planck's Constant
S=12    # I/I_s
dw=2*pi*13E6 # Frequency detunning
gama=2*math.pi*6E6 # Decay rate
dt=0.0001
t=0
fuck1=8*(h*(k**2)*dw*S)/(gama*(1+S+(2*dw/gama)**2)**2) # Spontaneous Force   for colling
dB_field=(NB*(3*u0*I*((coil_R)**2)*(hb))/((coil_R)**2+(hb)**2)**(2.5)) #  B-field Gradient
fuck2=dB_field*(0.5*ub*fuck1/(h*k))  #  B-field Trapping force ( # Like Spring Constant)
T_MOT=0
Th0=0
pp=0
Ke=0
...
scence1.range = L/4
gd = graph(title="<b><i>Number</i></b><sub>MOT</sub> - <b><i>time</i></b>", width=600, height=450, x=0, y=400,align='left',
               xtitle="<i>t</i> (ms)", ytitle="<i>Number</i> ")# Number
Coupling_ON = gcurve(graph=gd, color = color.blue)
gd2 = graph(title="<b>Temperature</b><sub>MOT</sub> - <b>time</b>", width=600, height=450, x=0, y=400,align='left',
               xtitle="<i>t</i> (ms)", ytitle="<i>T</i> (mK) ",ymin=0,ymax=1)# Temperature
TT= gcurve(graph=gd2, color = color.blue,label = "T<sub>exp</sub>")
Th= gcurve(graph=gd2, color = vec(1,0.7,0.2),label = "T<sub>th</sub> = 134 <i>u</i>K")
...
"""
for i in range(4):
    coil1=ring(pos=vec(0, 0.05+0.004*i, 0), axis=vec(0, 1, 0), radius=coil_R, thickness=0.25*Bs, color=vector(1,0.7,0.2),canvas=scence1)
for i in range(4):
    coil2=ring(pos=vec(0, -0.05-0.004*i, 0), axis=vec(0, 1, 0), radius=coil_R, thickness=0.25*Bs, color=vector(1,0.7,0.2),canvas=scence1)
"""
chamber
"""
d =L/4+Ratom
...
p = []
apos = []
pavg = sqrt(2*mass*1.5*K*T)*0.1 # average kinetic energy p**2/(2mass) = (3/2)kT
for i in range(Natoms):
    x = d*rd.uniform(-1, 1)
    y = d*rd.uniform(-1, 1)
    z = d*rd.uniform(-1, 1)
    if i == 0:
        Atoms.append(sphere(pos=vector(x,y,z), radius=Ratom, color=color.blue, make_trail=True, retain=10, trail_radius=0.3*Ratom,canvas=scence1,M=0))
...
b5 = button(text="Real", pos=scence1.title_anchor, bind=Real)
...
while True:
    ...
    if number>=1:
        ...
        Th.plot(pos=(t*1E3,134E-3))
        pp=pp/number
        Ke=pp/(2*mass)
        T_MOT=2*Ke/(3*number*K)
        
        pp = 0
    Coupling_ON.plot(pos=(t*1E3,number))
    ...
    for i in range(Natoms):
        if mag(apos[i])<=Bs :
            if mag(p[i])/mass<=0.131 and real:controllV1=0
            else:controllV1=1
            F_x = -controllV*controllV1*(fuck1*p[i].x/mass+0.5*fuck2*apos[i].x*controllB)
            F_y = -controllV*controllV1*(fuck1*p[i].y/mass+fuck2*apos[i].y*controllB)-mass*g
            F_z = -controllV*controllV1*(fuck1*p[i].z/mass+0.5*fuck2*apos[i].z*controllB)
            p[i].x += (F_x)*dt
            p[i].y += (F_y)*dt
            p[i].z += (F_z)*dt
        ...
        if(Atoms[i].color==color.cyan):pp= pp+mag2(p[i])
